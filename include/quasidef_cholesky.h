...
class QuasidefCholesky {
 public:
  QuasidefCholesky(int size, const int *Acolp, const int *Arows) : size(size) {
    ...
    build_tree(Acolp, Arows, parent, Lnz);
    ...
    build_nonzero_pattern(Acolp, Arows, parent, Lnz);
    delete[] parent;
    delete[] Lnz;
  ...
  */
  void init_supernodes(const int parent[], const int Lnz[], int vtn[]) {
    int snode = 0;
    for (int i = 0; i < size;) {
      vtn[i] = snode;
      i++;
